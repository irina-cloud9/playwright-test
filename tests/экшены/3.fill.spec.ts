...
test.describe('Заполнение базовых полей формы', () => {
  ...
  test('Заполнение email с валидацией', async ({ page }) => {
    ...
    await emailField.blur(); // Триггерим валидацию - убираем фокус с поля
    await expect(errorFeedback).toBeVisible();

    await emailField.fill('irina@mail.com');
...
test.describe('Заполнение специальных типов полей', () => {
  ...
  test('Заполнение числового поля', async ({ page }) => {
    ...
    //Пытаемся ввести нечисловое значение - это вызовет ошибку (Error: Cannot type text into input[type=number])
    //Для type="number" браузера блокирует такой ввод
    try {
      await ageField.fill('abc'); //Код, который может вызвать ошибку
    } catch (error) {
      // Код, который выполнится если в try произойдет ошибка
      // Ожидаемая ошибка - поле не принимает нечисловые значения
      console.log('Ожидаемая ошибка: поле type="number" не принимает текст');
      //Проверяем, что поле осталось пустым после попытки нечислового ввода
      await expect(ageField).toHaveValue('');
    }
  });
});
