...
test.describe('Базовые тесты для getByPlaceholder()', () => {
  ...
  test('Найти и заполнить поле по placeholder', async ({ page }) => {
    const nameInput = page.getByPlaceholder('Введите ваше имя'); //Поиск элемента input на странице по атрибуту placeholder/
    await nameInput.fill('Иван Иванов'); //Действие: заполнение найденного поля текстом "Иван Иванов"/
    await expect(nameInput).toHaveValue('Иван Иванов'); //Проверка утверждения: что поле содержит ожидаемое значение/
  });
  ...
  test('Найти поле по части placeholder', async ({ page }) => {
    ...
    await expect(emailInput).toHaveAttribute('type', 'email'); //<input type="email" placeholder="example@mail.com">/
  });
});
