...
test.describe('Базовые тесты для getByTitle()', () => {
  ...
  test('Найти элемент по точному title', async ({ page }) => {
    const tooltip = page.getByTitle('Это простая подсказка'); //Поиск элемента по точному совпадению атрибута title/
    await expect(tooltip).toBeVisible(); //Проверка, что элемент видим на странице/
    await expect(tooltip).toHaveClass('tooltip'); //Проверка, что элемент имеет CSS-класс 'tooltip'/
    await expect(tooltip).toHaveText('Наведи на меня'); //Проверка текстового содержимого элемента/
  });
...
test.describe('Тесты для ссылок и специальных случаев', () => {
  ...
  test('Найти ссылку по title', async ({ page }) => {
    ...
    await expect(homeLink).toHaveAttribute('href', '#'); //Проверка значения атрибута href у ссылки <a href="#" class="link-with-title" title="Перейти на главную страницу">Главная</a>/
    await expect(homeLink).toHaveClass('link-with-title'); //Проверка CSS-класса ссылки/
  });
...
test.describe('Сложные случаи и динамический контент', () => {
  ...
  test('Найти изображение по title', async ({ page }) => {
    ...
    await expect(image).toHaveJSProperty('width', 225); //проверяем размер изображения/
